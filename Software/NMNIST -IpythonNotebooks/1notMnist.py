...
# Given these sizes, it should be possible to train models quickly n any machine.
...
# Each exemplar should be an image of a character A through J rendered in a different font. Display a sample of the images that we just downloaded.
...
# we'll load each class into a separate dataset, store them on disk and curate them independently. Later we'll merge them into a single dataset of manageable size.
...
# The labels will be stored into a separate array of integers 0 through 9.Also create a validation dataset for hyperparameter tuning.
...
# Overlap between training and test can skew the results if you expect to use your model in an environment where there is never an overlap, but are actually
# ok if you expect to see training samples recur when you use it. Measure how much overlap there is between training, validation and test samples.
# Optional questions:
#
...
contador_eq = 0
#Between train dataset
for i in range(train_dataset.shape[0]):
    if not (i%500):
        print (i," ",contador_eq)
    sample_image = train_dataset[i, :, :]
    for j in range(train_dataset.shape[0]-1):
        if (j != i):
            sample_image_tc = train_dataset[j,:,:]
            if (np.array_equal(sample_image_tc,sample_image) == True):
                contador_eq = contador_eq + 1
print (contador_eq) # No se si anda
...
print (train_dataset.shape, "->", X.shape)
...
import time
t1 = time.time()
print('LogisticRegression score: %f' % logistic.fit(X,Y).score(Xtest,Ytest))
t2 = time.time()
